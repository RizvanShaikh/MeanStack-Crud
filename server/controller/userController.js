...
class Usercontroller {
    ...
    getallUser(req, res) {
        ...
        for (let i = 0; i < 5; i++) {
            // for if null value
            if (req.body['search'] && req.body['search']['value']) {
            //    console.log(req.body['search']['value'], "search value")
                if (req.body['colums'] && req.body['columns'][i]['data']) {
        ...
        option['offset'] = parseInt(req.body['start'] || 0);
        option['limit'] = parseInt(req.body['length'] || 10);
        // option['collation'] = { locale: "en_US", numericOrdering: true }

        userDao.findWithPeginate(query, option).then((data) => {
                // res.json({message:'coming',data});
            res.send({ status: 200, data: data })
        }).catch((err) => {
            res.send({ status: 400, message: err.message });
        });
